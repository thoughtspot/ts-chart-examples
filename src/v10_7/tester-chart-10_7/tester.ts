...
// To mock authentication
const TESTER_CHART_ACCESS_TOKEN = "custom-tester-chart-token";
...
async function renderKPIs(ctx: CustomChartContext) {
  ...
  const queries = chartConfigs.map(
    (config: ChartConfig): Query =>
      _.reduce(
        config.dimensions,
        (acc: Query, dimension) => ({
          queryColumns: [...acc.queryColumns, ...dimension.columns].filter(
  ...
  if (kpiBox) {
    const innerHTML = _.reduce(
      columns,
      (acc, colId, idx) => {
        const column = _.find(chartModel.columns, (col) => col.id === colId);
...
async function handleAxisMenuButtons(ctx: CustomChartContext) {
  ...
  document
    .getElementById("update-visual-props-button")
    ?.addEventListener("click", (e) => {
      ...
      if (typeof chartModel?.visualProps === "object") {
        ctx.emitEvent(ChartToTSEvent.UpdateVisualProps, {
          visualProps: {
            ...(chartModel?.visualProps || {}),
            clientState: "clientState updated",
          },
        });
      }
    });
...
async function handleLiveboardContextSection(ctx: CustomChartContext) {
  ...
  document.getElementById("context-status").textContent +=
    `${isLiveboardContext}`;
  document.getElementById("context-edit").style.opacity = isLiveboardContext
    ? ".4"
    : "1";
}
...
async function handleLocaleInfo(ctx: CustomChartContext) {
  const { locale = window.navigator.language } =
    ctx.getAppConfig().localeOptions;
...
const renderChart = async (ctx: CustomChartContext): Promise<void> => {
  ...
  try {
    // Mock authentication logic for playwright tests inside TS.
    // Should not be taken as a reference to develop authentication logic for custom charts.
    if (
      ctx.getAppConfig().chartAppAccessToken &&
      ctx.getAppConfig().chartAppAccessToken !== TESTER_CHART_ACCESS_TOKEN
    ) {
      throw new Error("Chart authentication failed.");
    }
